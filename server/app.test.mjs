...
test('support chat stores an isolated request and BOT 68 acknowledgement',async()=>{const owner=await register(102),other=await register(103),auth={authorization:`Bearer ${owner.body.token}`};const sent=await request('/api/support/messages',{method:'POST',headers:auth,body:JSON.stringify({category:'message',message:'Không gửi được tin nhắn Facebook',platform:'Windows',appVersion:'0.15.5'})});assert.equal(sent.status,201);assert.equal(sent.body.length,2);assert.equal(sent.body[0].sender,'user');assert.equal(sent.body[1].sender,'support');assert.ok(sent.body[1].message.includes('BOT68-'));const own=await request('/api/support/messages',{headers:auth}),hidden=await request('/api/support/messages',{headers:{authorization:`Bearer ${other.body.token}`}});assert.equal(own.body.length,2);assert.equal(hidden.body.length,0)})
...
test('Meta OAuth discovers Facebook and Instagram assets then connects selected channels',async()=>{
  ...
  const start=await request('/api/oauth/meta/start',{method:'POST',headers:auth,body:JSON.stringify({provider:'facebook'})});assert.equal(start.status,200)
  ...
  const status=await request(`/api/oauth/meta/status/${start.body.flowId}`,{headers:auth});assert.equal(status.body.status,'ready');assert.deepEqual(status.body.assets.map(a=>a.provider),['facebook'])
  const complete=await request('/api/oauth/meta/complete',{method:'POST',headers:auth,body:JSON.stringify({flowId:start.body.flowId,assetIds:status.body.assets.map(a=>a.id)})});assert.equal(complete.body.connected,1)
  const igStart=await request('/api/oauth/meta/start',{method:'POST',headers:auth,body:JSON.stringify({provider:'instagram'})}),igUrl=new URL(igStart.body.authorizeUrl)
  const igCallback=await fetch(`${base}/oauth/meta/callback?code=meta-code&state=${encodeURIComponent(igUrl.searchParams.get('state'))}`);assert.equal(igCallback.status,200)
  const igStatus=await request(`/api/oauth/meta/status/${igStart.body.flowId}`,{headers:auth});assert.deepEqual(igStatus.body.assets.map(a=>a.provider),['instagram'])
  const igComplete=await request('/api/oauth/meta/complete',{method:'POST',headers:auth,body:JSON.stringify({flowId:igStart.body.flowId,assetIds:igStatus.body.assets.map(a=>a.id)})});assert.equal(igComplete.body.connected,1)
...
test('tenant AI knowledge is isolated and produces grounded reviewable suggestions',async()=>{
  ...
  const greeting=await request('/api/ai/suggest',{method:'POST',headers:authA,body:JSON.stringify({customerName:'Lan',question:'Alo'})});assert.equal(greeting.status,200);assert.deepEqual(greeting.body.sourceIds,[document.body.id]);assert.ok(greeting.body.draft.includes('tư vấn'))
