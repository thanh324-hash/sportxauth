...
export function createApp(config) {
  ...
  app.get('/api/team',authenticate,(req,res)=>{res.json(db.prepare('SELECT * FROM users WHERE tenant_id=? ORDER BY created_at').all(req.session.tenantId).map(publicUser))})
...
function publicCustomer(row){return {id:row.id,name:row.name,phone:row.phone,email:row.email,channel:row.channel,externalId:row.external_id,tags:JSON.parse(row.tags||'[]'),note:row.note,createdAt:row.created_at,updatedAt:row.updated_at}}
...
function upsertSocialCustomer(db,tenantId,channel,event){if(event?.type!=='message'||!event.senderId)return;const now=Date.now(),externalId=String(event.senderId),name=String(event.senderName||`${channel} ${externalId}`);db.prepare("INSERT INTO customers(id,tenant_id,name,channel,external_id,tags,created_at,updated_at) VALUES(?,?,?,?,?,'[\"Khách mạng xã hội\"]',?,?) ON CONFLICT(tenant_id,channel,external_id) WHERE external_id IS NOT NULL AND external_id <> '' DO UPDATE SET name=excluded.name,updated_at=excluded.updated_at").run(id('cus'),tenantId,name,channel,externalId,now,now)}
