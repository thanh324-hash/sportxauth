...
export function createApp(config) {
  ...
  app.get('/api/support/messages',authenticate,(req,res)=>{const rows=db.prepare('SELECT sm.*,u.name AS user_name FROM support_messages sm JOIN users u ON u.id=sm.user_id WHERE sm.tenant_id=? ORDER BY sm.created_at').all(req.session.tenantId);res.json(rows.map(publicSupportMessage))})
  app.post('/api/support/messages',authenticate,(req,res)=>{const message=String(req.body?.message||'').trim(),category=String(req.body?.category||'general');if(!message||message.length>5000)return res.status(400).json({error:'Nội dung hỗ trợ phải từ 1 đến 5.000 ký tự'});if(!['general','login','channel','message','ai','billing','other'].includes(category))return res.status(400).json({error:'Nhóm hỗ trợ không hợp lệ'});const now=Date.now(),messageId=id('sup'),metadata={platform:String(req.body?.platform||'web').slice(0,50),appVersion:String(req.body?.appVersion||'').slice(0,30),userAgent:String(req.body?.userAgent||'').slice(0,300)};db.prepare('INSERT INTO support_messages(id,tenant_id,user_id,sender,category,message,metadata,created_at) VALUES(?,?,?,?,?,?,?,?)').run(messageId,req.session.tenantId,req.session.userId,'user',category,message,JSON.stringify(metadata),now);const reference=`BOT68-${new Date(now).toISOString().slice(0,10).replaceAll('-','')}-${messageId.slice(-6).toUpperCase()}`,replyId=id('sup');db.prepare('INSERT INTO support_messages(id,tenant_id,user_id,sender,category,message,metadata,created_at) VALUES(?,?,?,?,?,?,?,?)').run(replyId,req.session.tenantId,req.session.userId,'support',category,`BOT 68 đã nhận yêu cầu ${reference}. Thông tin nền tảng đã được đính kèm. Bộ phận hỗ trợ sẽ phản hồi trong cuộc trò chuyện này.`,JSON.stringify({reference,automatic:true}),now+1);const rows=db.prepare('SELECT sm.*,u.name AS user_name FROM support_messages sm JOIN users u ON u.id=sm.user_id WHERE sm.id IN (?,?) ORDER BY sm.created_at').all(messageId,replyId);res.status(201).json(rows.map(publicSupportMessage))})
  ...
  app.delete('/api/channels/:connectionId',authenticate,ownerOnly,async(req,res,next)=>{
    try{
      ...
      try{
        ...
        db.prepare('DELETE FROM sync_events WHERE tenant_id=? AND source_connection_id=?').run(req.session.tenantId,connection.id)
  ...
  app.post('/api/oauth/meta/start', authenticate, ownerOnly, (req, res) => {
    ...
    const requestedProvider=String(req.body?.provider||'facebook')
    if(!['facebook','instagram'].includes(requestedProvider))return res.status(400).json({error:'Kênh Meta không hợp lệ'})
    const flowId=id('oauth'),state=crypto.randomBytes(32).toString('base64url'),now=Date.now()
    const stateHash=crypto.createHash('sha256').update(state).digest('hex')
    db.prepare('INSERT INTO oauth_flows(id,tenant_id,user_id,provider,state_hash,created_at,expires_at) VALUES(?,?,?,?,?,?,?)').run(flowId,req.session.tenantId,req.session.userId,requestedProvider,stateHash,now,now+10*60*1000)
  ...
  app.get('/oauth/meta/callback', async(req,res)=>{
    ...
    try{
      ...
      db.exec('BEGIN');try{for(const page of accountsBody.data||[]){const pageToken=page.access_token||userToken;if(flow.provider==='facebook')insert.run(id('asset'),flow.id,flow.tenant_id,'facebook',String(page.id),String(page.name||page.id),encryptSecret(pageToken,config.encryptionSecret),null,'{}');if(flow.provider==='instagram'&&page.instagram_business_account){const ig=page.instagram_business_account;insert.run(id('asset'),flow.id,flow.tenant_id,'instagram',String(ig.id),String(ig.username||ig.name||ig.id),encryptSecret(pageToken,config.encryptionSecret),String(page.id),JSON.stringify({pageId:String(page.id),pageName:String(page.name||'')}))}}const count=db.prepare('SELECT COUNT(*) AS total FROM oauth_assets WHERE flow_id=?').get(flow.id).total;if(!count)throw new Error(flow.provider==='instagram'?'Không tìm thấy Instagram Professional đã liên kết với Page':'Không tìm thấy Facebook Page có thể quản lý');db.prepare("UPDATE oauth_flows SET status='ready' WHERE id=?").run(flow.id);db.exec('COMMIT')}catch(error){db.exec('ROLLBACK');throw error}
  ...
  app.post('/api/oauth/meta/complete',authenticate,ownerOnly,async(req,res)=>{
    ...
    const getAsset=db.prepare('SELECT * FROM oauth_assets WHERE id=? AND flow_id=? AND tenant_id=?'),upsert=db.prepare("INSERT INTO channel_connections(id,tenant_id,provider,external_id,display_name,encrypted_token,status,created_at,metadata) VALUES(?,?,?,?,?,?,'active',?,?) ON CONFLICT(tenant_id,provider,external_id) DO UPDATE SET display_name=excluded.display_name,encrypted_token=excluded.encrypted_token,status='active',metadata=excluded.metadata")
    let connected=0;const subscriptionAssets=[];db.exec('BEGIN');try{for(const assetId of selected){const asset=getAsset.get(assetId,flow.id,req.session.tenantId);if(asset){upsert.run(id('chn'),req.session.tenantId,asset.provider,asset.external_id,asset.display_name,asset.encrypted_token,Date.now(),asset.metadata||'{}');subscriptionAssets.push(asset);connected++}}db.prepare("UPDATE oauth_flows SET status='completed' WHERE id=?").run(flow.id);db.exec('COMMIT')}catch(error){db.exec('ROLLBACK');throw error}
    let subscribed=0;const subscriptionErrors=[]
    for(const asset of subscriptionAssets){try{const pageId=asset.provider==='instagram'?asset.parent_external_id:asset.external_id;const url=new URL(`https://graph.facebook.com/${config.metaGraphVersion}/${pageId}/subscribed_apps`);url.search=new URLSearchParams({subscribed_fields:'messages,message_deliveries,message_reads,message_echoes,messaging_postbacks,messaging_optins,messaging_referrals,feed',access_token:decryptSecret(asset.encrypted_token,config.encryptionSecret)}).toString();const response=await config.fetchImpl(url,{method:'POST'});const body=await response.json();if(!response.ok||body.success!==true)throw new Error(body.error?.message||'Meta khong chap nhan dang ky webhook');subscribed++}catch(error){subscriptionErrors.push({page:asset.display_name,error:error.message})}}
...
function publicSupportMessage(row){return {id:row.id,sender:row.sender,category:row.category,message:row.message,metadata:JSON.parse(row.metadata||'{}'),userName:row.user_name||'',createdAt:row.created_at}}
